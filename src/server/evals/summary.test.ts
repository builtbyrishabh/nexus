...
describe("summarize", () => {
  it("averages axes over answerable cases and passes when all bars are cleared", () => {
    const s = summarize([
  ...
  it("counts a wrongful refusal on an answerable case as zero, not as un-graded", () => {
    // Without this, a model that answers one case well and refuses the rest scores 100%.
    const s = summarize([
      answered("a", 0.9, 0.9, 0.9),
      { id: "b", expectRefusal: false, refused: true }, // answerable, wrongly refused → 0
    ]);
    expect(s.scored).toBe(2); // both answerable cases are in the denominator
    expect(s.means.faithfulness).toBeCloseTo(0.45); // (0.9 + 0) / 2
    expect(s.passed).toBe(false);
