...
export const env = createEnv({
  ...
  server: {
    ...
    /**
     * Rerank is off by default: it is the *measured* add-on (docs/DESIGN.md Slice 1 status), so
     * retrieve() behaves exactly as Slice 1 until this flips on. The eval harness overrides it
     * per-run to measure the lift rather than assert it.
     */
    ...
    /**
     * Optional YouTube Data API key. Unset → channel discovery uses the public RSS feed
     * (recent uploads, no key). Set → full uploads-playlist backfill becomes available.
     */
